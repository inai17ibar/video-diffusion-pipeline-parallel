...
- 動画生成AI (Stable Video Diffusion) をマルチGPUで高速化した
- **パイプライン並列**: diffusion stepsを複数GPUに分割し、流れ作業で処理
- デモ動画を見せる（入力画像→生成動画）

### 2. なぜパイプライン並列か（3分）

- 既存の並列化手法との比較図
  - **データ並列**: バッチを分割 → 推論では1枚ずつなので使えない
  - **テンソル並列**: モデルを分割 → 実装が複雑、all-reduce通信が重い
  - **パイプライン並列**: ステップを分割 → send/recvだけ、実装がシンプル
- SVDの特性（25-105ステップの逐次処理）がパイプラインと相性が良い理由

### 3. アーキテクチャ（3分）

- パイプライン図（GPU0→GPU1→...→GPU6、latentが流れる）
- 1本目 vs 定常状態の図
  - 充填フェーズ: GPUが順番に動く
  - 定常フェーズ: 全GPUが同時に別サンプルを処理
- コード構造の簡潔さ（`forward(latent, step) -> latent` + `send/recv`）

### 4. 実験結果: スケーリング（5分）— メインの見せ場

- **スループット表**: 1→3→5→7 GPUで線形スケーリング（7.28x）
- **1本目レイテンシ**: GPU数によらず一定（~32s）— パイプラインの本質的制約
- **GPU利用率の可視化**: 1台ずつ点灯していくパターン
- **損益分岐点**: 2本目から7GPUが勝つ

### 5. なぜ線形スケーリングが出たか — 理論分析（3分）

- 計算/通信比 = 14,500:1 → 通信は事実上ゼロ
- 最適ステージ数は通信ではなく「ステップ分割可能性」で決まる
- 他のモデル・環境ではどうなるか（マルチノード、軽量モデルとの比較）

### 6. 制約と今後（2分）

- GPU利用率の低さ（7台中1-2台しかアクティブでない）
- 改善の方向性: マイクロバッチ、非同期パイプライン
- 単発レイテンシは改善不可能 — ユースケースの選定が重要
...
| 実験 | 目的 | 備考 |
|------|------|------|
| 公式パイプラインとの品質比較 | 同一seed・同一パラメータで公式SVD pipelineと出力を並べ、ピクセル単位で一致するか確認。「正しさ」の証明 | `run_official_pipeline.py` が既にある。結果のdiffを取るだけ |
| ネイティブ解像度(1024×576)でのスケーリング | 320×576は実用的でない。本来の解像度でもスケーリングが維持されるか確認 | 7GPUなら動作確認済み。1GPUはOOMの可能性があるのでCFGなし等で条件を揃える工夫が必要 |
| CFG有効時のスループット比較 | CFG=3.0ではUNet 2回/stepになる。通信比は変わらないがstep時間が2倍に。スケーリング効率がさらに上がるはず | benchmark.pyにguidance_scale対応を追加 |

### 優先度 中（余裕があれば）

| 実験 | 目的 |
|------|------|
| steps/GPUを極限まで減らす実験 | 例: 25 steps / 25 GPU相当（シミュレーション）で、何steps/GPUで効率が劣化し始めるかを特定 |
| 生成動画のFIDやLPIPS | 品質を定量評価し、公式パイプラインとの差を数値化 |
| マイクロバッチ（パイプライン充填の改善） | 1本目レイテンシ削減のため、サンプルを小分けにして早期投入する手法の検証 |
